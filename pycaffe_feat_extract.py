...
def caffe_load_model(path_model_def, path_model, WITH_GPU = True):
  if WITH_GPU: caffe.set_mode_gpu()
  else: caffe.set_mode_cpu()
  print "loading model: ", path_model

  caffe_net = caffe.Classifier(path_model_def , path_model, image_dims = (224,224), raw_scale = 255, channel_swap=(2,1,0), mean = np.array([103.939, 116.779, 123.68]) )
  return caffe_net
...
if __name__ == '__main__':
  ...
  while True:
    ...
    print "decoding in %.2f sec."%(time.time()-start_time)

    # encode the top prediction
    top_predictions = Ys[0] # take predictions for the first (and only) image we passed in

    for tops, sents in enumerate(top_predictions):
      top_prediction = sents # these are sorted with highest on top
      candidate = ' '.join([ixtoword[ix] for ix in top_prediction[1] if ix > 0]) # ix 0 is the END token, skip that
      print '%dth PRED: (%f) %s' % (tops, top_prediction[0], candidate)
